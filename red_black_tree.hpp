...
class red_black_tree{
	public:
		...
		typedef tree_iterator<const Value>	const_iterator;
	...
	public:
		...
		red_black_tree() : _root(0), _val_alloc(allocator_type()), _node_alloc(node_allocator()), _compare(value_compare()), _size(0){
			init_nil_head();
			_root = _header;
		}

		red_black_tree(const red_black_tree& src) :  _compare(src._compare), _root(NULL) {
			*this = src;
		}
		template<class InputIt>
		red_black_tree(typename ft::enable_if< !ft::is_integral<InputIt>::value, InputIt >::type first, InputIt last,
