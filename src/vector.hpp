...
namespace ft
{
	...
	private:
		...
		size_type
		alloc_capacity(size_type size)
		{
			if (size > this->_capacity * 2 || this->_capacity == 0)
				return size;
			else if (size > this->_capacity && this->_capacity > 0)
				return this->_size * 2;
			else
	...
	public:
		...
		vector (const vector& x)
		{
			...
			this->_vector = this->_allocator.allocate(this->_size);
